...
const Kmedoid = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("http://127.0.0.1:5001/Kmedoid"); // Endpoint untuk Kmedoid
        console.log("API Response (KMedoid):", response.data);
        setData(response.data);
      } catch (error) {
        console.error("Error fetching KMedoid data:", error);
      }
    };
  ...
  return (
    <div className="p-6 bg-white">
      <h1 className="text-2xl font-bold text-center mb-6 text-black">
        Hasil Clustering Mahasiswa Dengan Metode K-Medoid
      </h1>
